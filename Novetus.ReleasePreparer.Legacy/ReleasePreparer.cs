...
namespace Novetus.ReleasePreparer.Legacy
{
    ...
    class ReleasePreparer
    {
        ...
        {
            string novpath = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) + @"\\Novetus_legacy";

            if (args.Length > 0)
            {
                if (args.Contains("-snapshot"))
                {
                    Console.WriteLine("Snapshots are unavailable.");
                }
        ...
        public static string GetBranch(INIFile ini, string infopath)
        {
            ...
            string novpath = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) + @"\\Novetus_legacy\\bin\\Novetus.exe";
